...
export type TaskDetailBackParams = {
  from?: string;
  visibility?: string;
  manager?: string;
};
...
/**
 * Shared task-detail body. Rendered by BOTH the full page (app/(app)/tasks/[id]/
 * page.tsx) and the intercepting modal (app/(app)/@modal/(.)tasks/[id]/page.tsx)
 * so the drawer and the direct URL show the exact same experience with a single
 * source of data fetching. No server-action or query logic changed here — this
 * is a straight extraction of the former page body.
 */
export async function TaskDetailContent({
  ...
  id: string;
  backParams?: TaskDetailBackParams;
}) {
  ...
  if (backParams.from === "admin-board") {
    ...
    if (backParams.manager) qs.set("manager", backParams.manager);
