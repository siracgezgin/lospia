  ...
  return (
    <div className="bg-surface rounded-card border border-line shadow-card p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-medium text-muted bg-surface-sunken rounded-full px-2 py-0.5 tabular-nums">
      ...
      {/* Effort selector */}
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-wider text-subtle mb-1.5">Efor</p>
        <div className="flex gap-2">
          {EFFORT_OPTIONS.map((e) => (
            <button
              ...
              className={cn(
                "flex-1 rounded-lg border px-3 py-2 text-sm transition-colors duration-150 active:scale-[0.98] disabled:opacity-60 disabled:pointer-events-none",
                effort === e
            ...
            >
              {EFFORT_LABELS[e]} <span className="text-[11px] text-subtle tabular-nums">({pointsForEffort(e)})</span>
            </button>
          ))}
        </div>
      ...
      {status === "review" && (
        <div className="anim-fade-up flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2.5">
      ...
      {status === "done" && (
        earned.length > 0 ? (
          <div className="anim-fade-up rounded-lg bg-green-50 border border-green-200 px-3 py-2.5 space-y-1">
