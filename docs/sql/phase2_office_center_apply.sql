-- =============================================================================
-- Lospia / AF Operasyon — Phase 2: Office Center (COMBINED apply script)
-- =============================================================================
-- DOKÜMANTASYON AMAÇLIDIR. Otomatik çalıştırılmaz.
--
-- Bu dosya şu migration'ların birebir kopyasıdır:
--   1) 20240207000000_office_center_foundation.sql
--   2) 20240208000000_task_participant_manage_rls.sql
--      (görev sorumluları RLS hizalaması — uygulama artık sorumlu yazımını
--       server-side service-role ile yapıyor, bu policy DB katmanını aynı
--       kurala hizalayan savunma katmanıdır; tablo/veri değiştirmez)
--
...
--   - mevcut tabloya dokunmaz; yalnızca 5 yeni tablo ve policy günceller
--   - "if not exists" / "drop policy if exists" ile idempotent
...
-- =============================================================================
-- 2) 20240208000000_task_participant_manage_rls.sql — birebir kopya
-- =============================================================================
-- Align task_member_completions RLS with the app assignment-permission model
-- (owner/admin any task; the task CREATOR their own task incl. initial
-- responsibles; a current responsible participant may hand off). The app now
-- performs this write server-side with the service-role client after the same
-- permission checks — this policy keeps direct client access consistent with
-- that rule. No schema change, no data change.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PostgREST schema cache yenile — bu satır atlanmamalıdır.
-- ─────────────────────────────────────────────────────────────────────────────
