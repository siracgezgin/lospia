-- =============================================================================
-- Lospia / AF Operasyon — Phase 3: Operasyonel Not Akışı (COMBINED apply script)
-- =============================================================================
-- DOKÜMANTASYON AMAÇLIDIR. Otomatik çalıştırılmaz.
--
-- Bu dosya şu migration'ın birebir kopyasıdır:
--   1) 20240209000000_task_note_workflow.sql
--      (görev notlarına not tipi / teslim tarihi onayı / aksiyon durumu
--       alanları + "Gördüm / Üzerime aldım" kayıt tablosu)
--
...
--   - mevcut tablolardan veri silmez, kolon düşürmez
--   - "if not exists" / "drop policy if exists" ile idempotent
--     (tekrar çalıştırmak güvenli)
--   - supabase db reset YOK
--
-- Bu SQL uygulanmadan da uygulama çalışır: not ekleme eski davranışa
-- (düz not) geri düşer, akış/rozet özellikleri kontrollü şekilde devre dışı
-- kalır. Tam özellik için bu script çalıştırılmalıdır.
--
