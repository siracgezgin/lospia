...
/**
 * KONU METNİNİ YERİNDE YAZAR — ızgaraya doğrudan, pencere açmadan.
 *
 * Aslı Hanım (2026-08-29): "Bu calendar kısmı biraz EXCEL TARZINDA olmalı."
 * Başlık ızgarada düzenlenebiliyordu ama konu için pencere açmak gerekiyordu;
 * oysa toplantı sırasında en çok yazılan şey konu ("Nisa burayı doldur").
 * Excel'de her hücreye yazılır — burada da öyle.
 *
 * Hücrede toplantı yoksa AÇILIR: boş bir "Konu 1"e yazmak da bir başlangıçtır
 * ve kullanıcıyı "önce toplantı oluştur" duvarına çarptırmak Excel hissini
 * bozar (moveTopic'teki aynı kural).
 *
 * Metin BOŞALTILIRSA konu silinmez, yalnız metni boşalır — silme ayrı ve
 * görünür bir eylemdir (konunun × düğmesi). Aslı Hanım bir kez veri kaybetti;
 * sessiz silme bir daha olmamalı.
 */
export async function setTopicText(
  target:
    | { topicId: string }
    | { meeting_date: string; time_slot: string; position: number },
  text: string,
): Promise<{ ok: true; topicId: string } | { error: string }> {
  ...
  if ("topicId" in target) {
    const { error, count } = await supabase
      .from("planning_topics")
      .update({ text: clean || null }, { count: "exact" })
    ...
    revalidatePath("/planning");
    revalidatePath("/home");
    return { ok: true, topicId: target.topicId };
  }
  ...
  const parsed = z
    ...
    .safeParse(target);
  if (!parsed.success) return { error: parsed.error.issues[0].message };
  if (!clean) return { error: "Konu boş olamaz." };

  // Hedef hücrenin toplantısı — yoksa aç (kategori komşusundan miras).
  ...
  let meetingId = (found as { id: string } | null)?.id ?? null;
  if (!meetingId) {
    const { data: sibling } = await supabase
      .from("planning_meetings")
      .select("category")
  ...
  /* Aynı satıra ikinci kez yazılırsa YENİ konu açılmaz, var olan güncellenir —
     yoksa hızlı yazan kullanıcı aynı pozisyonda iki konu üretiyordu. */
  ...
  revalidatePath("/planning");
  revalidatePath("/home");
  return { ok: true, topicId: (ins as { id: string }).id };
}
